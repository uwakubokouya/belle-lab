...
async function run() {
  const { data, error } = await s.rpc('exec_sql', { sql_query: "ALTER TABLE sns_profiles ADD COLUMN IF NOT EXISTS is_vip boolean DEFAULT false;" });
  console.log("Add col:", error);
}
run();
