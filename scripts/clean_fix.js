...
// 1. 変数追加
const tabRegex = /const \[activeTab, setActiveTab\] = useState<'timeline' \| 'gallery' \| 'shifts'>\('timeline'\);/;
...
// 2. useEffect 追加
const useEffectInsertRegex = /useEffect\(\(\) => \{\s*if \(isEditingProfile\)/;
const useEffectCode = `useEffect(() => {
  ...
  useEffect(() => {
    if (isEditingProfile)`;
content = content.replace(useEffectInsertRegex, useEffectCode);

// 3. fetchStoreInfoStr 
const storeInfoRegex = /(const weekDays = \["日", "月", "火", "水", "木", "金", "土"\];)/;
content = content.replace(storeInfoRegex, `$1
      const sid = storeCast?.store_id || 'ef92279f-3f19-47e7-b542-69de5906ab9b';
      const { data: storeProfile } = await supabase.from('profiles').select('full_name, username').eq('id', sid).maybeSingle();
      if (storeProfile) {
          let linkId = sid;
...
// 4. フォールバック処理
const castImgRegex = /let castName = profile\?\.name \|\| "";[\s\S]*?if \(!castName && storeCast\) \{\s*castName = storeCast\.name \|\| "";\s*\}/;
content = content.replace(castImgRegex, `let castName = profile?.name || "";
...
const setProfileDataRegex = /setProfileData\(prev => \(\{\s*\.\.\.prev,\s*name: castName,\s*image: castImg,\s*bio: castBio\s*\}\)\);/;
content = content.replace(setProfileDataRegex, `setProfileData(prev => ({ ...prev, name: castName, image: castImg, bio: castBio }));
...
content = content.replace(badgeRegex, `<h1 className="text-xl font-light tracking-[0.2em] mb-4">{profileData.name}</h1>
...
// 6. CAST DATA バッジと予約ボタンの非表示 (安全な置換)
const castDataRegexFull = /(<button onClick=\{\(\) => setShowPreferencesModal\(true\)\} className="px-4 py-1\.5 mb-2 border border-\[\#E5E5E5\] text-black bg-white hover:bg-\[\#F9F9F9\] transition-colors flex flex-col items-center justify-center tracking-widest gap-0\.5">\s*<span className="text-\[10px\] font-medium leading-none tracking-\[0\.1em\]">CAST<\/span>\s*<span className="text-\[8px\] font-bold leading-none tracking-\[0\.1em\]">DATA<\/span>\s*<\/button>)/g;
content = content.replace(castDataRegexFull, `{!(profileRole === "system" || profileRole === "store") && ($1)}`);

const ctaRegexFull = /(<Link href=\{`\/reserve\/\$\{id\}`\} className="premium-btn w-full flex items-center justify-center gap-3 py-4 text-sm tracking-widest">\s*<Calendar size=\{18\} className="stroke-\[1\.5\]" \/>\s*このキャストを予約する\s*<\/Link>)/g;
content = content.replace(ctaRegexFull, `{!(profileRole === "system" || profileRole === "store") && ($1)}`);

// 7. タブUIとコンテンツの置換
const tabBtnRegex = /<button\s*onClick=\{\(\) => setActiveTab\('shifts'\)\}\s*className=\{`flex-1 py-4 text-\[11px\] tracking-widest relative transition-colors \$\{activeTab === 'shifts' \? 'font-bold text-black bg-\[\#F9F9F9\]' : 'font-normal text-\[\#777777\] hover:bg-\[\#F9F9F9\]'\}`\}\s*>\s*出勤情報\s*\{activeTab === 'shifts' && <div className="absolute top-0 w-full h-\[1px\] bg-black"><\/div>\}\s*<\/button>/;
...
content = content.replace(tabBtnRegex, newTabsBlock);

const tabContentRegex = /\) : activeTab === 'shifts' \? \(/;
const newTabContentBlock = `) : activeTab === 'casts' ? (
              isLoadingCasts ? (
...
content = content.replace(tabContentRegex, newTabContentBlock);

fs.writeFileSync(file, content);
console.log('Clean fix completed successfully.');
