...
for (let i = 0; i < lines.length; i++) {
    ...
    // 5. isPrivileged の定義 (const actualCastId = profile ? profile.id : id;)
    // 既存の page.tsx では top level state ではないので、ここでは使わない。
    // その代わり、{(profileData.isAdmin || user?.id === id) && ( で囲む。
...
// 別ループで囲み処理を行う (行番号のズレを防ぐため後ろから処理するか、1回ずつbreakして再実行する)
function wrapBlock(keyword, startOffset, endFinder, wrapperStart, wrapperEnd, name) {
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(keyword)) {
            if (!lines[i+startOffset-1].includes(wrapperStart.trim())) {
